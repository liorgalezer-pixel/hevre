...
import { Mail, Lock, LockKeyhole, Eye, EyeOff, ChevronRight } from "lucide-react";
...
function PasswordRule({ met, label }: { met: boolean; label: string }) {
  return (
    <div className="flex items-center justify-end gap-2">
      ...
      <span className={`text-sm font-bold ${met ? "text-green-600" : "text-warm-danger"}`}>
        {met ? "✓" : "✗"}
      </span>
    </div>
  );
...
export default function RegisterPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-cream px-5 pt-14 pb-10" dir="rtl">
      ...
      {/* Title */}
      <h1 className="font-serif text-3xl font-bold text-ink mb-8 tracking-tight">יצירת חשבון</h1>
      ...
      <div className="flex flex-col gap-5 mb-8">
        ...
        <div>
          ...
          <div className={`bg-paper ring-1 rounded-2xl h-14 flex items-center px-4 gap-3 ${emailError ? "ring-warm-danger" : "ring-divider focus-within:ring-terracotta"} transition-colors`}>
            <Mail size={18} strokeWidth={1.6} className="text-ink-3 shrink-0" />
            <input
              type="text"
          ...
          {emailError && (
            <p className="text-xs text-warm-danger text-right mt-1.5">
        ...
        {/* Password */}
        <div>
          <label className="font-mono text-[11px] text-ink-3 uppercase tracking-wider block text-right mb-1.5">סיסמא</label>
          <div className="bg-paper ring-1 ring-divider focus-within:ring-terracotta rounded-2xl h-14 flex items-center px-4 gap-2 transition-colors">
            <button type="button" onClick={() => setShowPassword(!showPassword)} className="text-ink-3 min-w-[32px] flex items-center justify-center">
            ...
            <input
              ...
              dir="ltr"
            />
            <Lock size={18} strokeWidth={1.6} className="text-ink-3 shrink-0" />
          </div>
          {passwordTouched && (
        ...
        {/* Confirm Password */}
        <div>
          <label className="font-mono text-[11px] text-ink-3 uppercase tracking-wider block text-right mb-1.5">אימות סיסמא</label>
          <div className={`bg-paper ring-1 rounded-2xl h-14 flex items-center px-4 gap-2 transition-colors ${confirm.length > 0 && !safeEqual(confirm, password) ? "ring-warm-danger" : "ring-divider focus-within:ring-terracotta"}`}>
            <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="text-ink-3 min-w-[32px] flex items-center justify-center">
            ...
            <input
              ...
              dir="ltr"
            />
            <LockKeyhole size={18} strokeWidth={1.6} className="text-ink-3 shrink-0" />
          </div>
          {confirm.length > 0 && !safeEqual(confirm, password) && (
            <p className="text-xs text-warm-danger text-right mt-1.5">הסיסמאות לא תואמות</p>
          )}
        </div>
      </div>
      ...
      {/* Google button */}
      <button className="w-full bg-paper ring-1 ring-divider rounded-2xl h-14 flex items-center justify-center gap-3 active:bg-cream transition-colors">
      ...
      <p className="text-center text-sm text-ink-3 mt-6">
        כבר יש לך חשבון?{" "}
        <a href="/login" className="text-terracotta font-semibold">התחבר</a>
      </p>
    </div>
  );
}
